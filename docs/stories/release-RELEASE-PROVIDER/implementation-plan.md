...
- [~] Scenario: deployment_id ordering under apply -- Given a config where `labdeploy_e2e_test.deployment_id = labdeploy_deployment.x.id`, When `terraform apply` runs in a plan harness, Then Terraform Core orders the e2e_test create strictly after the deployment create [proof: service:tf-plugin-server; deps: terraform-plugin-testing harness runs the built provider under a real `terraform` binary and asserts apply ordering] — PARTIAL: the Core dependency EDGE is proven at plan time (`TestAccE2EDeploymentIDOrdersAfterDeployment` asserts the deployment's known-after-apply id propagates into `deployment_id`, unknown ⇒ edge). Observing the real Create SIDE-EFFECT sequence under a completed `apply` requires a live lab target (provider Create dials the host); per operator decision this full apply-ordering proof is DEFERRED to a dedicated integration workstream with real lab infra.
