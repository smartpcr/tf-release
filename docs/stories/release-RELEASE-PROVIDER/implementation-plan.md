...
- [x] Add the missing `files[].path` relative-path constraint (DESIGN.md line 323): `validate.go` lines 313-317 only reject an empty path; reject absolute paths and `..` traversal so a spec cannot write outside the release dir.
...
### Test Scenarios
- [x] Scenario: Rollback matrix rows -- Given a fake transport scripting a failure at each DESIGN sec 10.2 step, When `Deploy` runs, Then the resulting state matches the matrix row (staging wiped / fresh cleaned / update restored to previous) (T7) [proof: in-process; deps: none -- fake Transport with a scripted Result queue]
- [x] Scenario: Idempotent short-circuit -- Given a manifest whose version and checksum equal the spec and a healthy status, When `Deploy` runs, Then it returns NO-OP with no FETCH or SWITCH step logged [proof: in-process; deps: none -- fake Transport with a scripted Result queue]
- [x] Scenario: Single-host step order logged -- Given a successful single-host `Deploy` captured through a tflog test sink, When it completes, Then the emitted `step` values appear in the fixed **execution** order VALIDATE, CONNECT, PREFLIGHT, LOCK, FETCH, CHECKSUM, STAGE, EXTRACT, RENDER, STOP, SWITCH, CONFIGURE, START, HEALTH, FINALIZE, PRUNE, UNLOCK, and every emitted entry carries `app`, `host`, `step`, `version`, and a numeric `duration_ms` field [proof: in-process; deps: none -- tflog test sink asserting structured fields]
  - **Order resolution (DESIGN §9.2 is normative — OPERATOR-CONFIRMED):** the operator answered the `step-order-normative` question with choice **A** (DESIGN §9.2 execution order). The switchover sub-order is therefore `STOP → SWITCH → CONFIGURE → START` per the normative windows_service S-steps (§9.2: S1 STOP, S3 SWITCH, S4 CONFIGURE, S5 ENV, then START). DESIGN §8.5 line 486 (`…RENDER CONFIGURE STOP SWITCH START…`) is the fixed-step **name vocabulary catalog**, not an execution order; the earlier revision of this scenario mistakenly copied the catalog order. The engine emits, and `TestStepLogFreshSingleHostOrder` asserts (exactly once, exact order), the normative §9.2 execution order above. This decision is now closed (operator-pinned) — no open question remains.
- [x] Scenario: Conditional and per-host steps logged -- Given a multi-host update whose idempotency short-circuit skips FETCH/CHECKSUM on one host and a rollback repeats SWITCH, When captured through the tflog sink, Then each executed step (including repeated/per-host occurrences) still carries the full `app/host/step/version/duration_ms` field set and no conditional step is emitted when it was skipped [proof: in-process; deps: none -- tflog test sink asserting structured fields]
...
- [ ] Scenario: Coded diagnostic Summary -- Given a `PREFLIGHT` dial failure surfaced by the engine as the real taxonomy code `ERR_CONNECT` (transport.go `ErrConnect`), When `Create` surfaces it, Then the diagnostic Summary matches the `[<CODE>] <short>` shape (begins `[ERR_CONNECT] `) [proof: in-process; deps: none -- terraform-plugin-framework diagnostics unit test]
...
- [x] Scenario: Timeout kills tree -- Given `runner.timeout_seconds` exceeded, When `Create` runs via a fake transport, Then it returns `ERR_TIMEOUT` and issues the process-tree kill script [proof: in-process; deps: none -- fake Transport with a scripted Result queue]
...
- [x] Scenario: deployment_id ordering under apply -- Given a config where `labdeploy_e2e_test.deployment_id = labdeploy_deployment.x.id`, When `terraform apply` runs in a plan harness, Then Terraform Core orders the e2e_test create strictly after the deployment create [proof: service:tf-plugin-server; deps: terraform-plugin-testing harness runs the built provider under a real `terraform` binary and asserts apply ordering] — PROVEN: `TestAccE2EDeploymentIDOrdersAfterDeployment` runs a genuine `terraform apply` against the built provider (both resources' engines faked to succeed infra-free AND record when Core invokes each Create); after apply it asserts the recorded Create sequence is exactly `[deployment, e2e]`, observing both Create calls and comparing their order.
