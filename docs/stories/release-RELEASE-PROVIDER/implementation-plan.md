...
- [x] Scenario: Single-host step order logged -- Given a successful single-host `Deploy` captured through a tflog test sink, When it completes, Then the emitted `step` values appear in the fixed **execution** order VALIDATE, CONNECT, PREFLIGHT, LOCK, FETCH, CHECKSUM, STAGE, EXTRACT, RENDER, STOP, SWITCH, CONFIGURE, START, HEALTH, FINALIZE, PRUNE, UNLOCK, and every emitted entry carries `app`, `host`, `step`, `version`, and a numeric `duration_ms` field [proof: in-process; deps: none -- tflog test sink asserting structured fields]
  - **Order resolution (DESIGN §9.2 is normative):** the switchover sub-order is `STOP → SWITCH → CONFIGURE → START` per the normative windows_service S-steps (§9.2: S1 STOP, S3 SWITCH, S4 CONFIGURE, S5 ENV, then START). DESIGN §8.5 line 486 (`…RENDER CONFIGURE STOP SWITCH START…`) is the fixed-step **name vocabulary catalog**, not an execution order; the earlier revision of this scenario mistakenly copied the catalog order. The engine emits, and `TestStepLogFreshSingleHostOrder` asserts (exactly once, exact order), the normative §9.2 execution order above.
